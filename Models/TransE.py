...
class TransE():
	...
	def __init__(self, config):
		...
		head_cluster_differences = tensorflow.expand_dims(positive_head_embedding_vector, 1) - self.head_cluster_means
		...
		tail_cluster_differences = tensorflow.expand_dims(positive_tail_embedding_vector, 1) - self.tail_cluster_means
		...
		if config['original']:
			...
			#self.loss =  tensorflow.reduce_sum(tensorflow.maximum(positive - negative +  config['margin'], 0)) + (config['beta'] * self.cluster_cost)
	...
	def load_model(self, file_name):
		...
		'''
	def train_model(self, session, positive_heads, positive_tails, positive_relations, negative_heads, negative_tails, negative_relations): #, positive_distances
		...
		#print('cluster cost: ',cluster, '   loss: ', loss)
