...
class TMBD():
    ...
    def __init__(self):
        ...
        self.entity_count = 38603
        self.relation_count = 9
    
        if os.path.exists("Datasets/TMBD/train_data.npy"):
            self.train_data = numpy.load("Datasets/TMBD/train_data.npy")
        else:
            self.train_data = numpy.array(list(map(lambda x: numpy.array(list(map(int, x.split()))), open("Datasets/TMBD/train2id.txt").readlines()[1:]))) 
            numpy.save("Datasets/TMBD/train_data", self.train_data)
        '''
        if os.path.exists("TMBD_valid_data.npy"):
            self.valid_data = numpy.load("TMBD_valid_data.npy")
        else:
            self.valid_data = numpy.array(list(map(lambda x: numpy.array(list(map(int, x.split()))), open("TMBD/valid2id.txt").readlines()[1:]))) 
            numpy.save("TMBD_valid_data", self.valid_data)

        if os.path.exists("TMBD_test_data.npy"):
            self.test_data = numpy.load("TMBD_test_data.npy")
        else:
            self.test_data = numpy.array(list(map(lambda x: numpy.array(list(map(int, x.split()))), open("TMBD/test2id.txt").readlines()[1:]))) 
            numpy.save("TMBD_test_data", self.test_data)
        if os.path.exists("TMBD_all_data.pickle"):
            self.all_data = pickle.load(open("TMBD_all_data.pickle", "rb"))
        else:
            self.all_data = set(map(tuple, self.train_data.tolist() + self.valid_data.tolist() + self.test_data.tolist()))
            pickle.dump(self.all_data, open("TMBD_all_data.pickle", "wb"))
        if os.path.exists("TMBD_one_to_one_data.npy"):
            self.one_to_one_data = numpy.load("TMBD_one_to_one_data.npy")
        else:
            self.one_to_one_data = numpy.array(list(map(lambda x: numpy.array(list(map(int, x.split()))), open("TMBD/1-1.txt").readlines()[1:]))) 
            numpy.save("TMBD_one_to_one_data", self.one_to_one_data)

        if os.path.exists("TMBD_one_to_many_data.npy"):
            self.one_to_many_data = numpy.load("TMBD_one_to_many_data.npy")
        else:
            self.one_to_many_data = numpy.array(list(map(lambda x: numpy.array(list(map(int, x.split()))), open("TMBD/1-n.txt").readlines()[1:]))) 
            numpy.save("TMBD_one_to_many_data", self.one_to_many_data)

        if os.path.exists("TMBD_many_to_one_data.npy"):
            self.many_to_one_data = numpy.load("TMBD_many_to_one_data.npy")
        else:
            self.many_to_one_data = numpy.array(list(map(lambda x: numpy.array(list(map(int, x.split()))), open("TMBD/n-1.txt").readlines()[1:]))) 
            numpy.save("TMBD_many_to_one_data", self.many_to_one_data)

        if os.path.exists("TMBD_many_to_many_data.npy"):
            self.many_to_many_data = numpy.load("TMBD_many_to_many_data.npy")
        else:
            self.many_to_many_data = numpy.array(list(map(lambda x: numpy.array(list(map(int, x.split()))), open("TMBD/n-n.txt").readlines()[1:]))) 
            numpy.save("TMBD_many_to_many_data", self.many_to_many_data)        
        '''
        if os.path.exists("Datasets/TMBD/relation2id.pickle"):
            self.relation2id = pickle.load(open("Datasets/TMBD/relation2id.pickle", "rb"))
        else:
            self.relation2id = dict(map(lambda x: tuple([x.split()[0], int(x.split()[1])]), open("Datasets/TMBD/relation2id.txt").readlines()[1:]))
            pickle.dump(self.relation2id, open("Datasets/TMBD/relation2id.pickle", "wb"))
        '''
        if os.path.exists("TMBD_train_hierarchy.pickle"):
            self.train_hierarchy = pickle.load(open("TMBD_train_hierarchy.pickle", "rb"))
            self.train_root_entities = pickle.load(open("TMBD_train_root_entities.pickle", "rb"))
        else:
            self.train_root_entities, self.train_hierarchy = self.create_hierarchy(type_of_split = 'train')
            pickle.dump(self.train_hierarchy, open("TMBD_train_hierarchy.pickle", "wb"))
            pickle.dump(self.train_root_entities, open("TMBD_train_root_entities.pickle", "wb"))
            del self._children_of_all_entities

        if os.path.exists("TMBD_valid_hierarchy.pickle"):
            self.valid_hierarchy = pickle.load(open("TMBD_valid_hierarchy.pickle", "rb"))
            self.valid_root_entities = pickle.load(open("TMBD_valid_root_entities.pickle", "rb"))
        else:
            self.valid_root_entities, self.valid_hierarchy = self.create_hierarchy(type_of_split = 'valid')
            pickle.dump(self.valid_hierarchy, open("TMBD_valid_hierarchy.pickle", "wb"))
            pickle.dump(self.valid_root_entities, open("TMBD_valid_root_entities.pickle", "wb"))
            del self._children_of_all_entities

        if os.path.exists("TMBD_train_entity_distances.npy"):
            self.train_entity_distances = numpy.load("TMBD_train_entity_distances.npy")
        else:
            self.train_entity_distances = self.get_entity_distances(type_of_split = 'train')
            numpy.save("TMBD_train_entity_distances", self.train_entity_distances)

        if os.path.exists("TMBD_valid_entity_distances.npy"):
            self.valid_entity_distances = numpy.load("TMBD_valid_entity_distances.npy")
        else:
            self.valid_entity_distances = self.get_entity_distances(type_of_split = 'valid')
            numpy.save("TMBD_valid_entity_distances", self.valid_entity_distances)

        if os.path.exists("TMBD_bernoulli_probabilities.pickle"):
            self.bernoulli_probablities = pickle.load(open("TMBD_bernoulli_probabilities.pickle", "rb"))
        else:
            ...
            pickle.dump(self.bernoulli_probablities, open("TMBD_bernoulli_probabilities.pickle", "wb"))
...
if __name__ == '__main__':
    w = TMBD()
    pickle.dump(w, open("TMBD.pickle", "wb"))